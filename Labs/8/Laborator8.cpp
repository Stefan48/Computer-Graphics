...
void Laborator8::Init()
{
	...
	//Light & material properties
	{
		...
		lightDirection_step = 0.1f;
		materialShininess = 30;
		materialKd = 0.5;
		materialKs = 0.5;
		spot_light_active = false;
		spot_light_cutoff_angle = RADIANS(30);
		spot_light_cutoff_angle_step = 1.0f;
	}
}
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int spot_light_active_loc = glGetUniformLocation(shader->program, "spot_light_active");
	glUniform1i(spot_light_active_loc, (int)spot_light_active);

	int cutoff_angle = glGetUniformLocation(shader->program, "cutoff_angle");
	glUniform1f(cutoff_angle, spot_light_cutoff_angle);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		{
			spot_light_cutoff_angle -= spot_light_cutoff_angle_step * deltaTime;
		}
		...
		{
			lightDirection -= glm::vec3(lightDirection_step, 0.0f, 0.0f);
		...
		{
			lightDirection -= glm::vec3(0.0f, lightDirection_step, 0.0f);
			lightDirection = normalize(lightDirection);
		}
		...
		{
			lightDirection -= glm::vec3(0.0f, 0.0f, lightDirection_step);
			lightDirection = normalize(lightDirection);
		}
...
void Laborator8::OnKeyPress(int key, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		if (key == GLFW_KEY_F)
		{
			spot_light_active = !spot_light_active;
		}
	}
}
