...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	glm::ivec2 resolution = window->GetResolution();
	if (go_up1) {
		ty1 += 250 * deltaTimeSeconds;
		if (ty1 >= (float)resolution.y - 100) {
			ty1 = (float)resolution.y - 100;
			go_up1 = false;
		}
	}
	else {
	...
	// scale, then translate (written in reverse order)
	modelMatrix *= Transform2D::Translate(500, 350);
	// for scaling relative to square's center
	modelMatrix *= Transform2D::Translate(-(sx2 - 1) * 100 / 2, -(sy2 - 1) * 100 / 2);
	modelMatrix *= Transform2D::Scale(sx2, sy2);

	//modelMatrix *= Transform2D::Translate(500, 350) * Transform2D::Translate(-(sx2 - 1) * 100 / 2, -(sy2 - 1) * 100 / 2) * Transform2D::Scale(sx2, sy2);
	...
	//TODO create animations by multiplying current transform matrix with matrices from Transform 2D
	modelMatrix *= Transform2D::Translate(1000, 400) * Transform2D::Rotate(angle3);
	// for rotating around square's center
	modelMatrix *= Transform2D::Translate(-50, -50);
	//for rotating around another point
	//modelMatrix *= Transform2D::Translate(75, 75);
	angle3 += 1.5f * deltaTimeSeconds;
	if (angle3 > 6.2831f)
		//angle3 = 0;
		angle3 -= 6.2831f;
