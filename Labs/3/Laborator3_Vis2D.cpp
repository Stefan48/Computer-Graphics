...
void Laborator3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT)) {
		if (window->KeyHold(GLFW_KEY_W)) {
			logicSpace.y += 0.5f* deltaTime;
		}
		if (window->KeyHold(GLFW_KEY_S)) {
			logicSpace.y -= 0.5f* deltaTime;
		}
		if (window->KeyHold(GLFW_KEY_A)) {
			logicSpace.x -= 0.5f* deltaTime;
		}
		...
		if (window->KeyHold(GLFW_KEY_Z)) {
			...
			logicSpace.x += 0.25f * deltaTime;
			logicSpace.y += 0.25f * deltaTime;

			//logicSpace.width -= (initial_logic_width / 100.0f) * deltaTime;
			//logicSpace.height -= (initial_logic_height / 100.0f) * deltaTime;
		}
