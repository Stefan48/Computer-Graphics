...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix *= glm::translate(modelMatrix, glm::vec3(-0.5f, 0, 0.6f));
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_F)) {
			...
			if (!camera_ortho_active) {
				projectionMatrix = glm::perspective<float>(FoV, window->props.aspectRatio, zNear, zFar);
			}
		}
		...
		if (window->KeyHold(GLFW_KEY_R)) {
			// increase width for orthographic projection
			left -= (cameraSpeed / 2.0f) * deltaTime;
			right += (cameraSpeed / 2.0f) * deltaTime;
			if (camera_ortho_active) {
				projectionMatrix = glm::ortho<float>(left, right, bottom, top, zNear, zFar);
			}
		}
		if (window->KeyHold(GLFW_KEY_T)) {
			// decrease width for orthographic projection
			left += (cameraSpeed / 2.0f) * deltaTime;
			right -= (cameraSpeed / 2.0f) * deltaTime;
			if (camera_ortho_active) {
				projectionMatrix = glm::ortho<float>(left, right, bottom, top, zNear, zFar);
		...
		if (window->KeyHold(GLFW_KEY_Y)) {
			// increase height for orthographic projection
			bottom -= (cameraSpeed / 2.0f) * deltaTime;
			top+= (cameraSpeed / 2.0f) * deltaTime;
			if (camera_ortho_active) {
				projectionMatrix = glm::ortho<float>(left, right, bottom, top, zNear, zFar);
			}
		}
		if (window->KeyHold(GLFW_KEY_U)) {
			// decrease height for orthographic projection
			bottom += (cameraSpeed / 2.0f) * deltaTime;
			top -= (cameraSpeed / 2.0f) * deltaTime;
			if (camera_ortho_active) {
				projectionMatrix = glm::ortho<float>(left, right, bottom, top, zNear, zFar);
