...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data (cube)
	{
	...
	// Create a mesh box using custom data (pyramid)
	{
		vector<VertexFormat> vertices_pyr
		{
			// peak
			VertexFormat(glm::vec3(0, 1.5f, 0), glm::vec3(0.3, 0, 0.7)),
			VertexFormat(glm::vec3(0, 1.5f, 0), glm::vec3(1, 1, 0)),
			VertexFormat(glm::vec3(0, 1.5f, 0), glm::vec3(0, 0.7, 0.5)),
			VertexFormat(glm::vec3(0, 1.5f, 0), glm::vec3(0.8, 0.5, 0)),
			// 1st base vertex
			VertexFormat(glm::vec3(-1, 0, -1), glm::vec3(0.8, 0.5, 0)),
			VertexFormat(glm::vec3(-1, 0, -1), glm::vec3(0.3, 0, 0.7)),
			// 2nd base vertex
			VertexFormat(glm::vec3(-1, 0, 1), glm::vec3(0.3, 0, 0.7)),
			VertexFormat(glm::vec3(-1, 0, 1), glm::vec3(1, 1, 0)),
			// 3rd base vertex
			VertexFormat(glm::vec3(1, 0, 1), glm::vec3(1, 1, 0)),
			VertexFormat(glm::vec3(1, 0, 1), glm::vec3(0, 0.7, 0.5)),
			// 4th base vertex
			VertexFormat(glm::vec3(1, 0, -1), glm::vec3(0, 0.7, 0.5)),
			VertexFormat(glm::vec3(1, 0, -1), glm::vec3(0.8, 0.5, 0)),
			// base
			VertexFormat(glm::vec3(-1, 0, -1), glm::vec3(0.2, 0.2, 0.6)),
			VertexFormat(glm::vec3(-1, 0, 1), glm::vec3(0.2, 0.2, 0.6)),
			VertexFormat(glm::vec3(1, 0, 1), glm::vec3(0.2, 0.2, 0.6)),
			VertexFormat(glm::vec3(1, 0, -1), glm::vec3(0.2, 0.2, 0.6))
		};

		vector<unsigned short> indices_pyr =
		{
			0, 5, 6,
			1, 7, 8,
			2, 9, 10,
		...
		// Create a new mesh from buffer data
		Mesh *pyramid = CreateMesh("pyramid", vertices_pyr, indices_pyr);
	}

	// Create a mesh box using custom data (square)
	{
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube1"], shaders["VertexColor"], glm::vec3(-1.5f, 0.5f, 0), glm::vec3(0.25f)); // created using "InitFromData(vertices, indices)"

	// render object created using "Laborator2::CreateMesh"
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(1.0f, 0.5f, 1.0f), glm::vec3(0.2f));

	// render pyramid
	RenderMesh(meshes["pyramid"], shaders["VertexColor"], glm::vec3(0.0f, 0.75f, 0.0f), glm::vec3(0.35f));

	// rendere square
	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(-2.0f, 0.0f, 0.75f), glm::vec3(0.4f));
