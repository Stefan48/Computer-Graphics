...
void Laborator9::Update(float deltaTimeSeconds)
{
...

	{
		...
		// the square's orientation should follow the camera
		glm::vec3 camera_position = GetSceneCamera()->transform->GetWorldPosition();
		glm::vec3 dir = glm::normalize(position - camera_position);
		float rotx = -acos(glm::dot(dir, glm::vec3(dir.x, 0, dir.z)));
		float roty = acos(glm::dot(dir, glm::vec3(0, dir.y, dir.z)));
		modelMatrix = glm::rotate(modelMatrix, rotx, glm::vec3(1, 0, 0));
	...
	// Render quad with mixed textures
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(0.0f, 1.0f, -1.0f));
		modelMatrix = glm::scale(modelMatrix, glm::vec3(0.7f));
		mix_textures = true;
		RenderSimpleMesh(meshes["square"], shaders["ShaderLab9"], modelMatrix, mapTextures["bamboo"], mapTextures["random"]);
		mix_textures = false;
	}
}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	// Send mix_textures uniform
	glUniform1i(glGetUniformLocation(shader->program, "mix_textures"), (int)mix_textures);

	// Send application time (for earth texture only)
	if (texture1 && texture1 == mapTextures["earth"])
	{
