...
uniform int apply_noise;

// Output value to fragment shader
out vec3 color;
out float frag_apply_noise;
...
void main()
{
	...
	if (apply_noise != 0)
	{
		frag_apply_noise = 1.0;
	}
	else
	{
		frag_apply_noise = 0.0;
	}
	...
	if (apply_noise != 0)
	{
		...
		// compose both noises
		float displacement = - 10. * noise + b; // more pronounced noise
		//float displacement = -noise + b; // less pronounced noise

		// move the position along the normal and transform it
		vec3 newPosition = v_position + v_normal * displacement * 0.05f;
